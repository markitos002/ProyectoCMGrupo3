...
public List listarPagina(String pagina, int desde, int hasta) {
    ...
    String sql = "SELECT * FROM ("
            ...
            + "CI.HOCI_ID , "
            + "CONCAT(HC.HOCI_HORAINICIO, ':' ,HC.HOCI_MINUTOINICIO) HORA, "
            + "CI.PERS_IDMEDICO , "
            + "CONCAT(P2.PERS_PRIMERNOMBRE,' ', P2.PERS_SEGUNDONOMBRE,' ', P2.PERS_PRIMERAPELLIDO,' ', P2.PERS_SEGUNDOAPELLIDO) MEDICO,   " 
            + "CI.CONS_ID , "
            + "CONCAT('[',CO.CIUD_NOMBRE, '] ', CO.CONS_DIRECCION, ' [', CO.CONS_NOMBRE, ']') LUGAR, "
            + "CI.TICI_ID , TC.TICI_NOMBRE, CI.CITA_OBSERVACION , "
            + "CI.CITA_REGISTRADOPOR , CI.CITA_FECHACAMBIO , "
            + "CI.FECHA , CI.CITA_MOTIVODECONSULTA , ROW_NUMBER() OVER (ORDER BY CI.CITA_ID) AS CONT FROM APPCONSULTAS.CITAS CI, "
            + "APPCONSULTAS.MEDICO ME, APPCONSULTAS.PACIENTE PA, APPCONSULTAS.PERSONA P1, APPCONSULTAS.PERSONA P2, (SELECT CI.CIUD_NOMBRE, CO.* FROM APPCONSULTAS.CONSULTORIO CO, APPCONSULTAS.CIUDAD CI WHERE CO.CIUD_ID = CI.CIUD_ID) CO,   "  
            ...
            + "CI.PERS_IDMEDICO = ME.PERS_ID AND ME.PERS_ID = P2.PERS_ID AND CI.CONS_ID = CO.CONS_ID AND CI.HOCI_ID = HC.HOCI_ID AND CI.TICI_ID = TC.TICI_ID AND POSITION( ? IN CI.PERS_IDMEDICO)>0 ORDER BY CI.CITA_ID) AS TABLA WHERE CONT>=? AND CONT<=? ORDER BY CONT";
    try {
...
Citas citas = (Citas) dat;
    ...
    try{
        con = cnx.getConnection();
        sql = "INSERT INTO APPCONSULTAS.CITAS ( PERS_IDPACIENTE , HOCI_ID , PERS_IDMEDICO , CONS_ID , TICI_ID , CITA_OBSERVACION , CITA_REGISTRADOPOR , CITA_FECHACAMBIO , FECHA , CITA_MOTIVODECONSULTA ) VALUES ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? )";
        //JOptionPane.showMessageDialog(null, sql, "Resultado de la Operación", JOptionPane.INFORMATION_MESSAGE);
        ps = con.prepareStatement(sql);
...
public int calcularPaginas(String parametro) {
    int nroRegistros = 0;
    int paginador = 0;
    String sql = "Select count(*) from appconsultas.citas where fecha like ? ";
    try {
        con = cnx.getConnection();
        ps = con.prepareStatement(sql);
        ps.setString(1, "%"+parametro+"%");
