...
public List listar() {
    ...
    String sql = "SELECT PE.PERS_ID, CONCAT(PE.PERS_PRIMERAPELLIDO,' ',PE.PERS_SEGUNDOAPELLIDO,' ',PE.PERS_PRIMERNOMBRE,' ',PE.PERS_SEGUNDONOMBRE) NOMBRE, "
            + "PE.PERS_MAIL, RO.ROL_NOMBRE, US.USUA_ID, US.ROPE_ID, RO.ROL_ID, RO.ROL_PAGINAINICIO, US.USUA_USUARIO, US.USUA_CONTRASENA "
    ...
    try{
        ...
        while (rs.next()) {
            ...
            usuario.setPaginaInicio(rs.getString("ROL_PAGINAINICIO"));
...
public int calcularPaginas(String parametro) {
    int nroRegistros = 0;
    int paginador = 0;
    String sql = "Select count(*) from appconsultas.usuario where usua_usuario like ? ";
    try {
        con = cnx.getConnection();
        ps = con.prepareStatement(sql);
        ps.setString(1, "%"+parametro+"%");
